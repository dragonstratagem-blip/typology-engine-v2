...
    st.session_state.vals = {'dof': 2, 'data': [[0,0,0,0] for _ in range(4)]}
...
        # Explicitly labeled inputs

        inputs[f"lp{i}"] = st.selectbox(f"{labels[i]} L-Pol", ["+", "-"], index=st.session_state.vals['data'][i][0])

        inputs[f"mp{i}"] = st.selectbox(f"{labels[i]} M-Pol", [" ", "+", "-"], index=st.session_state.vals['data'][i][1])

        inputs[f"mm{i}"] = st.selectbox(f"{labels[i]} M-Mag", ["1", "2", "3"], index=st.session_state.vals['data'][i][2])

        inputs[f"sp{i}"] = st.selectbox(f"{labels[i]} S-Pol", [" ", "+", "-"], index=st.session_state.vals['data'][i][3])

        inputs[f"sm{i}"] = st.selectbox(f"{labels[i]} S-Mag", ["1", "2", "3", "4", "5", "6"], index=st.session_state.vals['data'][i][4])
...
        letter = mapping[label][inputs[f"lp{i}"]]
