...
for i, col in enumerate(cols):
    with col:
        ...
        inputs[f"m{i}"] = st.selectbox(f"{labels[i]} Magnitude", ["1", "2", "3"], index=st.session_state.vals['data'][i][1])
        inputs[f"sp{i}"] = st.selectbox(f"{labels[i]} S-Pol", [" ", "+", "-"], index=st.session_state.vals['data'][i][2])
        inputs[f"sm{i}"] = st.selectbox(f"{labels[i]} S-Mag", ["1", "2", "3", "4", "5", "6"], index=st.session_state.vals['data'][i][3])
...
if st.button("Generate"):
    ...
    for i, label in enumerate(labels):
        ...
        html_output += apply_styles(letter, inputs[f"p{i}"], inputs[f"m{i}"], inputs[f"sp{i}"], inputs[f"sm{i}"], str(st.session_state.vals['dof']))
