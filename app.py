...
        inputs[f"lp{i}"] = st.selectbox(f"{labels[i][:2]} Letter-Polarity (+ or -)", ["+", "-"], index=st.session_state.vals['data'][i][0])

        inputs[f"mp{i}"] = st.selectbox(f"{labels[i][:2]} Influence-Polarity (+ or- or nul)", [" ", "+", "-"], index=st.session_state.vals['data'][i][1])

        inputs[f"mm{i}"] = st.selectbox(f"{labels[i][:2]} Influence-Magnitude (1-3)", ["1", "2", "3"], index=st.session_state.vals['data'][i][2])

        inputs[f"sp{i}"] = st.selectbox(f"{labels[i][:2]} Capacity-Polarity (+ or- or nul)", [" ", "+", "-"], index=st.session_state.vals['data'][i][3])

        inputs[f"sm{i}"] = st.selectbox(f"{labels[i][:2]} Capacity-Magnitude (1-6)", ["1", "2", "3", "4", "5", "6"], index=st.session_state.vals['data'][i][4])
