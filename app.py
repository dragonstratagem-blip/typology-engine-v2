...
# --- Sidebar Inputs (Moved to top so they define values for the logic below) ---
with st.sidebar:
...
# --- Main Page Execution (At the top) ---
if st.button("Randomize All"):
    st.session_state.vals['dof'] = random.randint(0, 4)
    st.session_state.vals['data'] = [[random.randint(0,1), random.randint(0,1), random.randint(0,2), random.randint(0,2), random.randint(0,5)] for _ in range(4)]
    st.rerun()
